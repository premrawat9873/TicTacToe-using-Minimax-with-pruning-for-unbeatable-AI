...
public class AlphaBetaPruning {
	...
	static int findBestMove(int[] currentState) {
		...
		for(int i=0;i<tempState.length;i++) {
			if(tempState[i]==0) {
				tempState[i]=2;
				int score = Minimax(tempState,0, Integer.MIN_VALUE, Integer.MAX_VALUE, false);
				tempState[i]=0;
				if(score > bestMove) {
					bestMove = score;
					nextMove = i;
	...
	static int Minimax(int[] currentState, int dept, int alpha, int beta, boolean isMaximizing) {
		nodeCount++;
		if((currentState[0] == 1 && currentState[1] == 1 && currentState[2] == 1) ||
		           (currentState[0] == 1 && currentState[4] == 1 && currentState[8] == 1) ||
		           (currentState[0] == 1 && currentState[3] == 1 && currentState[6] == 1) ||
			...
			return possibleScores[1]; //-1
		           ...
		           (currentState[3] == 2 && currentState[4] == 2 && currentState[5] == 2) ||
		           (currentState[6] == 2 && currentState[7] == 2 && currentState[8] == 2)) {
			return possibleScores[2]; //1
		}
		else if(checkTieCondition(currentState)) {
			return possibleScores[0]; //0
		}
		else {
			if(isMaximizing) {
				int[] tempState = Arrays.copyOf(currentState, currentState.length);
				...
				for(int i = 0 ; i<tempState.length ; i++) {
					if(tempState[i]==0) {
						tempState[i]=2;
						int score = Minimax(tempState,dept+1, alpha, beta, false);
						tempState[i]=0;
						if(score>bestScore) {
							bestScore = score;
						}
				...
				for(int i = 0 ; i<tempState.length ; i++) {
					if(tempState[i]==0) {
						tempState[i]=1;
						int score = Minimax(tempState,dept+1, alpha, beta, true);
						tempState[i]=0;
						if(score<bestScore) {
							bestScore = score;
						}
